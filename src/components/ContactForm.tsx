    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          <input name="firstName" type="text" placeholder="Jane" required
            className="w-full px-4 py-3 rounded-lg border border-[#e8e8e8] text-sm text-[#222222] placeholder-[#6a6a6a] focus:outline-none focus:border-[#2A7C6F] focus:ring-2 focus:ring-[#2A7C6F]/10 transition" />
        </div>
        <div>
      ...
      <div>
        ...
        <select name="interest" required
          className="w-full px-4 py-3 rounded-lg border border-[#e8e8e8] text-sm text-[#222222] focus:outline-none focus:border-[#2A7C6F] focus:ring-2 focus:ring-[#2A7C6F]/10 transition bg-white">
      ...
      <div>
        ...
        <textarea name="message" rows={5} required placeholder="Tell me a little about what you're looking for…"
          className="w-full px-4 py-3 rounded-lg border border-[#e8e8e8] text-sm text-[#222222] placeholder-[#6a6a6a] focus:outline-none focus:border-[#2A7C6F] focus:ring-2 focus:ring-[#2A7C6F]/10 transition resize-none" />
      </div>

      <button type="submit" disabled={status === 'sending'}
        className="w-full bg-[#222222] text-white font-semibold text-sm py-3.5 rounded-lg hover:bg-[#2A7C6F] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
        {status === 'sending' ? 'Sending…' : 'Send Message'}
      </button>

      {status === 'success' && (
        <div className="bg-[#e8f5f2] text-[#2A7C6F] text-sm font-medium px-4 py-3 rounded-lg text-center">
